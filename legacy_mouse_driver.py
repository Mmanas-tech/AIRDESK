...
class MouseController:
    ...
    def move_mouse(self, x, y):
        ...
        # Use settings.SCREEN_MARGIN to create a "active area" in the camera frame
        # x is from 0 to settings.FRAME_WIDTH
        # y is from 0 to settings.FRAME_HEIGHT
        
        # Linear Interpolation: np.interp(v, [min_in, max_in], [min_out, max_out])
        
        screen_x = np.interp(x, 
                             [settings.SCREEN_MARGIN, settings.FRAME_WIDTH - settings.SCREEN_MARGIN], 
                             [0, self.screen_width])
        
        screen_y = np.interp(y, 
                             [settings.SCREEN_MARGIN, settings.FRAME_HEIGHT - settings.SCREEN_MARGIN], 
        ...
        speed_factor = min(dist / settings.SMOOTHING_SPEED_THRESHOLD, 1.0)
        
        # Invert: High speed = Low smoothing value
        # smooth_val = MAX - (MAX - MIN) * speed_factor
        current_smooth = settings.MAX_SMOOTHING - (settings.MAX_SMOOTHING - settings.MIN_SMOOTHING) * speed_factor
    ...
    def scroll(self, steps):
        ...
        if steps > 0:
            pyautogui.scroll(int(settings.SCROLL_SPEED))
        else:
            pyautogui.scroll(int(-settings.SCROLL_SPEED))
