...
class HandTracker:
    def __init__(self, mode=False, max_hands=settings.MAX_NUM_HANDS, 
                 detection_con=settings.MIN_DETECTION_CONFIDENCE, 
                 track_con=settings.MIN_TRACKING_CONFIDENCE):
    ...
    def draw_landmarks(self, img, landmarks):
        ...
        for p1, p2 in self.HAND_CONNECTIONS:
            if p1 < len(points) and p2 < len(points):
                cv2.line(img, points[p1], points[p2], settings.COLOR_HAND_LINES, settings.UI_THICKNESS)
        
        # Draw points
        for cx, cy in points:
             cv2.circle(img, (cx, cy), 5, settings.COLOR_HAND_POINTS, cv2.FILLED)
