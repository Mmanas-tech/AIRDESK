...
from gesture_v3.engine_constants import GestureState
...
class CinematicHUD:
    ...
    def draw(self, img, hand_landmarks, state, confidence):
        ...
        for i in range(1, len(self.trail)):
            ...
            cv2.line(img, self.trail[i - 1], self.trail[i], engine_constants.COLOR_IDLE, thickness)

        # 2. Draw Reticle (Rotating Circles)
        self.pulse_phase += 0.1
        radius = int(40 + np.sin(self.pulse_phase) * 5)

        color = engine_constants.COLOR_IDLE
        if state == GestureState.MOVE:
            color = engine_constants.COLOR_MOVE
            radius = 20
        elif state == GestureState.CLICK_LEFT:
            color = engine_constants.COLOR_CLICK
            radius = 35
            cv2.circle(img, (cx, cy), radius, color, 3)
        elif state == GestureState.CLICK_RIGHT:
            color = engine_constants.COLOR_RIGHT_CLICK
            radius = 35
            cv2.putText(img, "R", (cx - 10, cy + 10), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
        elif state == GestureState.DRAG_ACTIVE:
            color = engine_constants.COLOR_DRAG_ACTIVE
            radius = 35
            cv2.circle(img, (cx, cy), radius, color, 4)
            cv2.rectangle(img, (cx - 10, cy - 10), (cx + 10, cy + 10), color, -1)
        elif state == GestureState.FIST:
            color = self.COLOR_FIST
            radius = 30
            cv2.putText(img, "TOGGLE", (cx - 25, cy - 40), cv2.FONT_HERSHEY_PLAIN, 1, color, 1)
        elif state == GestureState.SCROLL:
            color = engine_constants.COLOR_SCROLL
            radius = 45
        ...
        # Confidence bar (shown when confidence > 0)
        if confidence > 0:
