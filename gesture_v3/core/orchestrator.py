...
class SystemController:
    ...
    def run(self):
        ...
        smoother = OneEuroFilter(time.time(), [0.5, 0.5], min_cutoff=engine_constants.ONE_EURO_MIN_CUTOFF, beta=engine_constants.ONE_EURO_BETA)
        ...
        while self.running:
            ...
            if fps < engine_constants.FAILSAFE_FPS and (current_time - self.start_time) > 2.0:
                 cv2.putText(img, "SAFETY PAUSE: LOW FPS", (engine_constants.WINDOW_WIDTH//2 - 150, engine_constants.WINDOW_HEIGHT//2), 
                ...
                # B. NORMAL MODE (Not Dragging)
                    ...
                    if state == "MOVE":
                        ...
                        if (current_time_loop - self.last_click_time) > engine_constants.CLICK_COOLDOWN:
                             ...
                             cv2.circle(img, (int(norm_x*engine_constants.WINDOW_WIDTH), int(norm_y*engine_constants.WINDOW_HEIGHT)), 50, engine_constants.COLOR_CLICK, 4)
